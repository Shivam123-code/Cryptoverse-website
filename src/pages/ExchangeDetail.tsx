    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="p-6 bg-gray-900 border border-gray-800 rounded-lg">
          <div className="flex items-center mb-4 space-x-2 text-red-500">
          ...
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="flex items-center px-4 py-2 space-x-2 text-white transition-colors bg-blue-500 rounded-lg hover:bg-blue-600 disabled:opacity-50"
          >
    ...
    <div className="px-4 py-8 mx-auto max-w-7xl">
      <div className="p-6 bg-gray-900 border border-gray-800 rounded-lg">
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2">
          ...
          <div className="p-6 bg-gray-800 rounded-lg">
            <h2 className="mb-4 text-xl font-bold">Exchange Info</h2>
            <div className="space-y-2">
              ...
              {exchange.description && (
                <p className="mt-4 text-sm text-gray-400">{exchange.description}</p>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Trading Pairs */}
        <div className="p-6 bg-gray-800 rounded-lg">
          <h2 className="mb-6 text-xl font-bold">Trading Pairs</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-gray-700">
                  <th className="px-4 pb-4">Pair</th>
                  <th className="px-4 pb-4">Price</th>
                  <th className="px-4 pb-4">Volume (24h)</th>
                  <th className="px-4 pb-4">Trust Score</th>
                </tr>
              </thead>
              <tbody>
                {exchange.tickers?.slice(0, 50).map((ticker: any, index: number) => (
                  <tr key={index} className="border-b border-gray-700">
                    ...
                    <td className="px-4 py-4">${ticker.last?.toFixed(2) || 'N/A'}</td>
                    <td className="px-4 py-4">${ticker.volume?.toFixed(2) || 'N/A'}</td>
                    <td className="px-4 py-4">
